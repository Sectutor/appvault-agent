        ...
        if n == 0:
            ...
            conn.execute("INSERT INTO profiles (name, identity, created, updated) VALUES (?,?,?,?)",
                         ("Default", legacy, datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                          datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
            conn.commit()
        if not _cfg_get("active_profile"):
            _cfg_set("active_profile", "Default")
    except Exception:
        pass  # never let an import-time seed failure kill the module
    conn.close()
...
def _slash_cron(args):
    ...
    if m_add:
        ...
        return "⚠️ Usage: /cron add <name>|<schedule>|<task> — e.g. /cron add MyJob|daily|Summarize the vault"
    parts = [p.strip() for p in args.split("|")]
    m_del = re.match(r"^del\s+(\d+)$", args)
    if m_del:
    ...
    m_run = re.match(r"^run\s+(\d+)$", args)
    if m_run:
        cid = int(m_run.group(1))
        conn = _db()
        row = conn.execute("SELECT * FROM cron_jobs WHERE id=?", (cid,)).fetchone()
        conn.close()
        if not row:
